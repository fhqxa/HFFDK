...
def setup_run(arg_mode='train'):
    ...
    if not args.no_wandb:
        wandb.init(project=f'HFFDK-{args.dataset}-{args.way}w{args.shot}s',
                   config=args,
                   save_code=True,
                   name=args.extra_dir)
...
def compute_TIE(tr, p_nd, r_nd):
    ...
    for i in range(len(p_nd)):
        r_anc = tree_ANcestor(tr, r_nd[i])
        p_anc = tree_ANcestor(tr, p_nd[i])
        b = list(set(r_anc).difference(set(p_anc)))
        c = list(set(p_anc).difference(set(r_anc)))
        TIE = TIE + len(b + c)
    # r_anc = tree_ANcestor(tr, r_nd)
    # p_anc = tree_ANcestor(tr, p_nd)
    # b = list(set(r_anc).difference(set(p_anc)))
    # c = list(set(p_anc).difference(set(r_anc)))
    # TIE = len(b + c)
    return TIE
...
def compute_FH(tr, p_nd, r_nd):
    ...
    for i in range(length):
        r_anc = tree_ANcestor(tr, r_nd[i])
        p_anc = tree_ANcestor(tr, p_nd[i])
        b = [x for x in r_anc if x in p_anc]
        PH = len(b) / len(p_anc)
        RH = len(b) / len(r_anc)
        FH = 2 * PH * RH / (PH + RH)
        sum_PH = sum_PH + PH
...
def parse_args(arg_mode):
    parser = argparse.ArgumentParser(description='Hierarchical few-shot learning with feature fusion driven by data and knowledge')

    ''' about dataset '''
    parser.add_argument('-dataset', type=str, default='tieredimagenet',
                        choices=['miniimagenet', 'cub', 'tieredimagenet', 'cifar_fs', 'cifar_fc', 'voc'])
    parser.add_argument('-data_dir', type=str, default='/home/grcwoods/WZP/Datas/tiered_imagenet/', help='dir of datasets')
    ...
    parser.add_argument('-extra_dir', type=str, default='FC_bs_5w5s', help='extra dir name added to checkpoint dir')         
