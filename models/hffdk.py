...
from models.resnetIGLF import ResNet
...
class HFFDK(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        self.fc = nn.Linear(self.encoder_dim, self.args.num_class)    #num_fine_class
        # self.fc = nn.Linear(self.encoder_dim, self.args.num_fine_class)  # num_fine_class
        self.c_fc = nn.Linear(self.encoder_dim, self.args.num_coarse_class)  # num_fine_class
    ...
    def _make_scr_layer(self, planes):
        ...
        if self.args.self_method == 'scr':
            # iaaf_block = iIGLF(channels=self.encoder_dim)
            # ms_cam = MS_CAM(channels=self.encoder_dim)
