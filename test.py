...
from common.utils import compute_accuracy, load_model, setup_run, set_seed, by, compute_TIE, compute_FH, create_array, pred_coarse_label
...
def evaluate(epoch, model, loader, args=None, set='val'):
    ...
    with torch.no_grad():
        for i, (data, labels, train_coarse_labels) in enumerate(tqdm_gen, 1):
            ...
            logits, logits_coarse= model((data_shot.unsqueeze(0).repeat(args.num_gpu, 1, 1, 1, 1), data_query))
